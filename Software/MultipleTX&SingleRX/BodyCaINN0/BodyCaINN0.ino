...
volatile bool FristTrigger = false;
...
boolean checkRxStatus() {
  ...
  while (true) {
    ...
    if (currentRxState != initialState) {  // State has changed
      ...
      if (elapsedTime < THRESHOLD_BUSY_1) {
        return RX_BUSY;  // Busy: state change time is less than 50 microseconds
      } else if (elapsedTime < THRESHOLD_BUSY_2) {
        return RX_BUSY;  // Busy: state change time is between 50 and 100 microseconds
      }
...
void setup() {
  ...
  Serial.begin(250000);
  Serial.println("Welcome to SeTH");
...
// #define HighDurationThreshold  10    // High level threshold (microseconds)
// #define LowDurationThreshold 15     // Low level threshold (microseconds)
// #define TransitionTimeout 40       // Transition detection timeout (microseconds)
// boolean rxBUSY = true;
...
// void setup() {
//   Serial.begin(250000);
//   Serial.println("Welcome to SeTH0");
//   pinMode(CaINRxGPIO,INPUT); 
//   pinMode(CaINTxRxSwitch,OUTPUT); 
//   pinMode(CaINOPWMSW, OUTPUT);
//   pinMode(buttonPin, INPUT);
...
// // Initialize the rx
//   Serial.println("NODE0 Start Listening!");
//   //Disable_CarrierPWM();
//   digitalWriteDirect(CaINOPWMSW, LOW); 
//   digitalWriteDirect(CaINTxRxSwitch, HIGH);
// }
